...
double wid(double M, double L ,int cha){
  ...
  ///AEM = 0.00789417635; /// M = 400, L = 6000; 
  ///note in pythia AEM is running 
  // M = 3000 0.0080791821
  /// M = 2000 0.00804125914
  
  ///running aem. 
  AEM =  pyalem(SH);

  //cout<<"M: "<<M<<" alpha "<<AEM<<" EE ( sqrt(4pi*alpah): "<<sqrt(4*3.141592653589793*AEM)<<endl;
...
double widci(double M,double L){
...
double widci_noCorr(double M,double L){
...
// ////12 channels. 
// double width_all_channels(double M, double L,int cha, int exLepType){
//   if(cha<1 || cha>15){
//     cout<<" chanele: "<<cha<<endl;
//     exit(1);
//   }
//    ////from pywidth.f
...
//   double WDTP_CI = 1/(96*PI)*pow(M,5)/pow(L,4); 
...
float getBranchRatioMuStarMG(float M,float Lambda){
  ...
  float br =wid(M,Lambda,1)/(wid(M,Lambda,2)+wid(M,Lambda,3)+wid(M,Lambda,1)+widci(M,Lambda));
...
float corrFacbrlz(float M,float Lambda){
  ...
  float br =(wid(M,Lambda,2)+wid(M,Lambda,3)+wid(M,Lambda,1))/(wid(M,Lambda,2)+wid(M,Lambda,3)+wid(M,Lambda,1)+widci(M,Lambda));
...
float brlz(float M,float Lambda){
  ...
  float br =wid(M,Lambda,2)/(wid(M,Lambda,2)+wid(M,Lambda,3)+wid(M,Lambda,1)+widci(M,Lambda));
...
float brCI(float M,float Lambda){
  ...
  float br =widci(M,Lambda)/(wid(M,Lambda,2)+wid(M,Lambda,3)+wid(M,Lambda,1)+widci(M,Lambda));
...
float brnw(float M,float Lambda){
  ...
  float br =wid(M,Lambda,3)/(wid(M,Lambda,2)+wid(M,Lambda,3)+wid(M,Lambda,1)+widci(M,Lambda));
...
float brci_ee(float M,float Lambda){
  ...
  float br =widci_ee(M,Lambda)/(wid(M,Lambda,2)+wid(M,Lambda,3)+wid(M,Lambda,1)+widci(M,Lambda));
...
float brci_mutau(float M,float Lambda){
  ...
  float br =widci_mutau(M,Lambda)/(wid(M,Lambda,2)+wid(M,Lambda,3)+wid(M,Lambda,1)+widci(M,Lambda));
...
float brci_qq(float M,float Lambda){
  ...
  float br =widci_qq(M,Lambda)/(wid(M,Lambda,2)+wid(M,Lambda,3)+wid(M,Lambda,1)+widci(M,Lambda));
...
//total XS at Lambda = 10 TeV from pythia8 for Mass 200,400,600,800,1000, 1500,2000,
float Lambda0 = 10; 
...
double searchCS(int j,double xlimt){
  ...
  for(double L = Lstart; L<=50; L += 1){
    ...
    if(useCorrDecay==1) br= wid(M,L1*1000,1)/(wid(M,L1*1000,2)+wid(M,L1*1000,3)+wid(M,L1*1000,1)+widci(M,L1*1000));
    else br = wid(M,L1*1000,1)/(wid(M,L1*1000,2)+wid(M,L1*1000,3)+wid(M,L1*1000,1)+widci_noCorr(M,L1*1000));
    double xsbr1 = xs*br; 
    xs = Xsec_Sig[j]*pow(Lambda0,4)/pow(L2,4);
    if(useCorrDecay==1) br= wid(M,L2*1000,1)/(wid(M,L2*1000,2)+wid(M,L2*1000,3)+wid(M,L2*1000,1)+widci(M,L2*1000));
    else br = wid(M,L2*1000,1)/(wid(M,L2*1000,2)+wid(M,L2*1000,3)+wid(M,L2*1000,1)+widci_noCorr(M,L2*1000));
    double xsbr2 = xs*br; 
  ...
  for(double L = L1; L<= L2; L += stepsize){
    ...
    if(useCorrDecay==1) br= wid(M,L,1)/(wid(M,L,2)+wid(M,L,3)+wid(M,L,1)+widci(M,L));
    else br = wid(M,L,1)/(wid(M,L,2)+wid(M,L,3)+wid(M,L,1)+widci_noCorr(M,L));
